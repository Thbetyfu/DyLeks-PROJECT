...
:: 1. Membersihkan proses zombie di port 3001 & 3002 sebelum memulai
echo  [1/4] Membersihkan port 3001 (Frontend) dan 3002 (Backend)...
...
start /b cmd /c "cd /d "%BE_DIR%" && python -m uvicorn app.main:app --host 0.0.0.0 --port 3002" >nul 2>&1
...
start /b cmd /c "cd /d "%FE_DIR%" && npm run dev" >nul 2>&1
...
echo.
echo  Status Server:
echo   - Frontend PWA: http://localhost:3001 (Port 3001)
echo   - Backend API : http://localhost:3002 (Port 3002)
echo.
echo  Petunjuk Kelas:
